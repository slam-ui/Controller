...
3. **CI/CD:**
    ...
    - ⚠️ Secrets для keystore (необходимо добавить в GitHub)
...
**⚠️ Важно:**
- Keystore НЕ должен быть в git (добавлен в .gitignore)
- Пароль должен быть в переменных окружения
- В production использовать настоящие сертификаты от CA
...
### ⚠️ Для production дополнительно требуется:

- Rate limiting
- Валидация входных данных
- Логирование безопасности
- CORS настройка
- Настоящие SSL сертификаты от CA
- Secret rotation
...
1. Keystore не включен в репозиторий (нужно генерировать локально)
2. В примерах используется `--insecure` для curl (self-signed сертификат)
3. Секреты должны быть добавлены в GitHub Secrets для CI/CD
4. Валидация входных данных базовая
5. Нет пагинации для списков
